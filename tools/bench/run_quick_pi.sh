...
restore() {
    ...
    echo "# udp2raw-wutong.service: $(systemctl is-active udp2raw-wutong.service 2>/dev/null); wg1 latest handshake: $(wg show wg1 latest-handshakes 2>/dev/null | awk -v now="$(date +%s)" '{print now-$2 "s ago"}' | paste -sd,)"
